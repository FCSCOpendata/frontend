  ...
  return (
    <>
      <div className="relative w-full bg-gray-200 rounded-lg overflow-hidden group min-h-[100px]">
        ...
        <div className={`absolute py-4 bottom-0 inset-x-0 text-white text-m ${AR('pr-5', 'pl-5')} leading-4 flex justify-between group-hover:bg-slate-200 group-hover:opacity-75 group-hover:text-black transition-all items-center`}>
          <h3 className="font-avenir font-bold">{title}</h3>
          {!!icon.url && (
            <div className={`block overflow-hidden w-[40px] h-[40px] ${AR('ml-4', 'mr-4')}`}>
              <img
                src={icon.url}
                alt={icon.alt}
                className="block"
